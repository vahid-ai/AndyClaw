...
/**
 * [LlmClient] for Google Gemini models via Vertex AI's OpenAI-compatible endpoint.
 *
 * Authenticates using a Google Cloud service account JSON key.
 * Uses [OpenAiFormatAdapter] for message format conversion and
 * [OpenAiStreamAccumulator] for streaming, since the Vertex AI endpoint
...
class VertexAiClient(
    ...
    private fun buildEndpointUrl(projectId: String, location: String = "us-central1"): String =
    ...
    /**
     * Creates a signed JWT for the service account and exchanges it for an access token.
     */
    private suspend fun getAccessToken(): String = tokenMutex.withLock {
        val now = System.currentTimeMillis()
        cachedAccessToken?.let { token ->
            if (now < tokenExpiresAtMs - TOKEN_REFRESH_BUFFER_MS) return token
        }
